<template>
  ...
  <div class="card">
    <Button label="Create New User" icon="pi pi-plus" class="btn-primary" @click="createUserModal = true" />
...
export default {
  ...
  data() {
    return {
      ...
      editIcon: '<svg width="13" height="13" viewBox="0 0 13 13" fill="none" xmlns="http://www.w3.org/2000/svg">\n' +
          ...
          '</svg>',
      deleteIcon: '<svg width="15" height="16" viewBox="0 0 15 16" fill="none" xmlns="http://www.w3.org/2000/svg">\n' +
  ...
  methods: {
    async getUsersList() {
      await UsersService.getUsersData().then(data => {
        this.usersList = data;
      }).catch(err => {
        console.error(err);
      });
